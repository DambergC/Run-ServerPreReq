...
function Install-NotepadPP {
    # Check if Notepad++ is installed via registry
    $regPaths = @(
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*",
        "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*"
    )
    ...
    if ($nppInstalled) {
        Write-Log "Notepad++ is already installed." "SUCCESS"
    } else {
        Write-Log "Notepad++ not found. Installing via winget..." "WARN"
        # You can change the ID if you want a portable or preview version
        winget install Notepad++.Notepad++
        Write-Log "Notepad++ installed via winget." "SUCCESS"
    }
}
